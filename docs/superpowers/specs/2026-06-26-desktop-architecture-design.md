...
- 保留 CLI 端入口，原 CLI 能力迁入新架构后复用同一套配置、提取服务和任务运行时。
...
- 第一版不重做复杂交互式 CLI，只保留脚本化和自动化可用的 CLI 入口。
...
apps/
  ...
  cli/            原 CLI 端迁入这里，复用 packages 的服务和配置
  web/            未来 Web 端占位
...
`domain` 不依赖 Electron、React、SQLite 或文件系统。Electron、CLI、数据库、文件系统和 LLM API 都是外层适配器。原有 CLI 能力不再维持一套独立业务实现，而是通过 `apps/cli` 调用 `packages/extraction`、`packages/jobs`、`packages/config`、`packages/llm` 和 `packages/tools`。未来迁移 Web 时，替换本地文件系统和 SQLite 适配器即可复用前端、领域模型、任务模型和图谱模块。
...
第一版内置供应商预设只包含：`自定义`、`MiniMax`、`DeepSeek`、`GLM`、`Kimi`、`Xiaomi MiMo`、`Qwen（通义千问）`。每个供应商都通过配置暴露多个模型。DeepSeek 第一版模型示例包含 `deepseek-v4-pro` 和 `deepseek-v4-flash`；其他供应商的具体模型列表从 provider preset schema 读取，不写死在提取业务里。
...
预设供应商使用 CC Switch 式胶囊按钮排列，只显示第一版允许的供应商，不展示 cc-switch 里其他第三方代理服务。选择官方预设后自动填充供应商名称、官网链接、默认请求地址、模型预设和能力标记；选择 `自定义配置` 时需要用户手动填写必要字段。
...
高级选项默认折叠，参考 CC Switch 的处理方式。第一版可以展示折叠入口，但只把必要配置写入 schema：协议格式、完整 URL 模式、模型测试配置和计费配置。普通用户不需要展开高级选项也能完成 DeepSeek、MiniMax、GLM、Kimi、Xiaomi MiMo 和 Qwen 的配置。
...
- 用户可以通过右上用户下拉进入大模型配置弹窗，启用供应商、保存 API key 引用、维护多个模型并选择默认模型。
- 大模型配置全程使用 CC Switch 风格弹窗，不使用右侧抽屉或独立页面。
...
- 没有模型配置时，提取页引导用户配置模型。
- `apps/cli` 可以复用同一套配置和提取服务运行原 CLI 的核心提取流程。
...
- CLI 入口测试：`apps/cli` 调用共享配置、分章、任务和写入服务。
...
- 上传一本小说并创建试跑任务。
- 配置 DeepSeek 的 `deepseek-v4-pro` 和 `deepseek-v4-flash`，确认提取页能选择。
